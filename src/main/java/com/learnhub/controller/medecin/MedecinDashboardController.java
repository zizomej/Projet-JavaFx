...
public class MedecinDashboardController {
    ...
    private void loadData(int medecinId) {
        try {
            ...
            Set<Integer> creneauIds = creneaux.stream()
                    .map(Creneau::getId)
                    .collect(Collectors.toSet());
            var rdvList = new RdvDAO().findAll().stream()
                    .filter(rdv -> creneauIds.contains(rdv.getCreneauId()))
                    .toList();
    ...
    @FXML private void goDashboard() { navigate("/fxml/medecin/dashboard.fxml", "Espace Médecin"); }
