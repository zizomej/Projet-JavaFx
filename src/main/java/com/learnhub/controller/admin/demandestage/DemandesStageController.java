...
public class DemandesStageController {
    ...
    private void showDetails(DemandeStage demande) {
        ...
        navigate("/fxml/admin/demandestage/demande_stage_details.fxml", "Détails Demande de Stage");
    }
    ...
    private void updateStatut(DemandeStage demande, String statut) {
        try {
            ...
            // -1 = pas de score (le score vient de Affinda, disponible depuis détail page)
            ctrl.initData(demande, statut, popupStage, this::loadData, -1, null);
            popupStage.showAndWait();
        } catch (Exception e) {
            e.printStackTrace();
            // fallback si le popup ne charge pas
            new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.ERROR,
                "Erreur popup : " + e.getMessage()).show();
        }
    }
