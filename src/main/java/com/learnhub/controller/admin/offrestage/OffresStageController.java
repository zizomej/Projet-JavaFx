package com.learnhub.controller.admin.offrestage;
...
public class OffresStageController {
    ...
    private void handleDeleteItem(OffreStage offre) {
        try {
            ...
            if (controller.isConfirmed()) {
                // CASCADE: supprime d'abord toutes les demandes liées
                try { demandeDao.deleteByOffre(offre.getId()); } catch (Exception ignored) {}
    ...
    private void showDialog(OffreStage offre, boolean edit) {
        try {
            javafx.fxml.FXMLLoader loader = new javafx.fxml.FXMLLoader(getClass().getResource("/fxml/admin/offrestage/offre_stage_form.fxml"));
            javafx.scene.Parent root = loader.load();
