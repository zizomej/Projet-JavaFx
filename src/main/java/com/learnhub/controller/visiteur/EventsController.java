...
public class EventsController {
    ...
    private VBox createEventCard(Evenement e) {
        ...
        dateBadge.setStyle(
                "-fx-background-color: linear-gradient(to bottom right, #EFF6FF, #DBEAFE); -fx-border-color: #BFDBFE; -fx-border-width: 1; -fx-border-radius: 10; -fx-background-radius: 10; -fx-padding: 10 18;");

        String day = e.getDateDebut() != null ? String.valueOf(e.getDateDebut().getDayOfMonth()) : "?";
        String month = e.getDateDebut() != null
                ? e.getDateDebut().format(DateTimeFormatter.ofPattern("MMM")).toUpperCase()
                : "?";
        ...
        monthLabel.setStyle(
                "-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: #3B82F6; -fx-letter-spacing: 1px;");
        ...
        typeLabel.setStyle(
                "-fx-font-size: 12px; -fx-font-weight: 800; -fx-text-fill: #64748B; -fx-letter-spacing: 1.5px;");
        ...
        String locStr = (e.getLieuNom() != null && !e.getLieuNom().isEmpty()) ? "📍 " + e.getLieuNom()
                : "📍 Lieu à confirmer";
